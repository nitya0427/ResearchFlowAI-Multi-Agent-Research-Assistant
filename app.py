...
# ── Page config ───────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="ResearchFlow AI - Multi-Agent Research Assistant",
    page_icon="🚀", 
    layout="wide",
...
.hero-wrap::before {
    ...
    background: linear-gradient(90deg, transparent, #4F8CFF, #8B5CF6, transparent);
}
.hero-title {
...
.pipe-node.active:not(:last-child)::after { background: #4F8CFF; }
.pipe-dot {
...
.pipe-node.active .pipe-dot  { border-color: #4F8CFF; background: #111B3D; box-shadow: 0 0 12px #4F8CFF40; }
...
.pipe-node.active .pipe-label { color: #4F8CFF; }
...
.stTextInput > div > div > input:focus {
    ...
    box-shadow: 0 0 0 2px #4F8CFF25 !important;
}
...
.stButton > button:hover { background: linear-gradient(90deg,#6AA8FF,#A78BFA) !important; transform: translateY(-1px); box-shadow: 0 4px 20px #4F8CFF30 !important; }
...
.card-writer::before  { background: #4F8CFF; }
...
.badge-writer { background: #111B3D; color: #4F8CFF; border: 1px solid #253D7A; }
...
@keyframes glow {
    0%, 100% { opacity: 1; box-shadow: 0 0 8px #4F8CFF; }
    50%       { opacity: 0.4; box-shadow: 0 0 3px #4F8CFF; }
}
</style>
...
<div class="footer">
    <div class="footer-inner">
        ...
        <div class="footer-text">Created by &nbsp;<span>Nitya Bachchhas</span></div>
        <div class="footer-dot"></div>
